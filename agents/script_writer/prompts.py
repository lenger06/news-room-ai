...
- Add [BROLL: url | description] markers at 1–3 natural visual moments where b-roll should appear behind the anchor. B-roll source rules — read carefully:
  ...
  * FALLBACK: if neither section has a suitable entry for a moment, write [BROLL: vivid visual description] with NO URL — the system searches for a still image using that description. Make the description specific and visual, like "Ukrainian soldiers in a trench" or "smoke rising over Kyiv skyline" — NOT generic phrases like "search query" or "news footage". Do NOT use article source links, thumbnail URLs, or any URL from the body of the research as b-roll. Only URLs from the two sourced sections above are valid b-roll URLs.
