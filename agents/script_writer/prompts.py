...
- Add [BROLL: url | description] markers at 1–3 natural visual moments where b-roll should appear behind the anchor. Source from the "## SOURCED B-ROLL IMAGES" and "## SOURCED B-ROLL VIDEOS" sections in the research brief — copy URLs and descriptions exactly. For video clips add a "| video" type suffix: e.g. [BROLL: https://example.com/clip.mp4 | cargo ship sailing | video]. For still images omit the suffix: e.g. [BROLL: https://example.com/photo.jpg | Met Gala red carpet 2026]. If no sourced media fits, use [BROLL: search query] (no URL) and the system will search for a still image. Place each marker immediately before the first word of the new subject — this includes any transitional bridge sentence ("Turning now to...", "In other news...", "Meanwhile..."). The B-roll switches the instant the marker is reached, so do NOT put bridging language before the marker; put it after. Write 2–4 sentences after each marker for meaningful dwell time.
